...
STOP LOSS BEHAVIOUR:
  SOL_STOP_LOSS=0.55       → fixed absolute price
  SOL_STOP_LOSS_OFFSET=0.05→ dynamic = avg_price - 0.05 (recalculates on DCA)
  SOL_STOP_LOSS=null  AND
  SOL_STOP_LOSS_OFFSET=null→ stop loss = avg_price (break-even protection)
...
.env variables:
    ...
    SOL_STOP_LOSS          (fixed price | null → break-even mode)
    SOL_STOP_LOSS_OFFSET   (dynamic offset | null)
    SOL_BET_STEP           (null | float)
    SOL_POLL_INTERVAL      (seconds between ticks, used for DCA/SL polling)
...
ENTRY_PRICE    = float(os.getenv("SOL_ENTRY_PRICE",    "0.70"))
AMOUNT_PER_BET = float(os.getenv("SOL_AMOUNT_PER_BET", "2.50"))
TAKE_PROFIT    = float(os.getenv("SOL_TAKE_PROFIT",    "0.95"))
POLL_INTERVAL  = float(os.getenv("SOL_POLL_INTERVAL",  "1.0"))

_bet_step_raw  = os.getenv("SOL_BET_STEP",          "null").strip().lower()
BET_STEP: Optional[float] = None if _bet_step_raw == "null" else float(_bet_step_raw)

_sl_raw        = os.getenv("SOL_STOP_LOSS",         "null").strip().lower()
STOP_LOSS: Optional[float] = None if _sl_raw == "null" else float(_sl_raw)

_sl_offset_raw = os.getenv("SOL_STOP_LOSS_OFFSET",  "null").strip().lower()
...
def wait_for_active_market() -> dict:
    log.info("Searching for active SOL 15M market ...")
    while True:
        ts = get_current_window_timestamp()
        for candidate in [ts, ts + 900]:
            slug   = f"sol-updown-15m-{candidate}"
