"""
strategies/YES+NO_1usd/btc/bot.py
----------------------------------
...
.env variables:
  BTC_PRICE_RANGE        e.g. "0.40-0.45"
  BTC_AMOUNT_TO_BUY      USDC amount per side (e.g. 1.0)
  BTC_POLL_INTERVAL      seconds between price checks (e.g. 0.5)
...
log = logging.getLogger("BTC-YES+NO")
...
_range_raw        = os.getenv("BTC_PRICE_RANGE", "0.40-0.45")
PRICE_RANGE       = _parse_range(_range_raw)
AMOUNT_TO_BUY     = float(os.getenv("BTC_AMOUNT_TO_BUY",  "1.0"))
POLL_INTERVAL     = float(os.getenv("BTC_POLL_INTERVAL",   "0.5"))
...
def run(interval: Optional[str] = None):
    ...
    range_low, range_high = PRICE_RANGE
    log.info("=" * 60)
    log.info("BTC YES+NO Strategy starting")
